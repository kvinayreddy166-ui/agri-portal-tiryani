...
function drawFormJ(cursor: PdfCursor, values: FertilizerPdfValues) {
  ...
  field(cursor, '(9) Name and Address of Fertilizer Inspector Drawing Sample', formatFormKAddressLines(values.inspectorNameAddress), 82, PAGE.marginX, {
    ...fieldOptions,
    gap: 1.2,
  });
...
function formatFormKAddressLines(value: string): string {
  const lines = value.split('\n');
  const lastNonEmptyIndex = lines.reduce((last, line, index) => (line.trim() ? index : last), -1);
  if (lastNonEmptyIndex < 0) return value;
...
function drawFormK(cursor: PdfCursor, formType: 'K_ADA' | 'K_JDA', values: FertilizerPdfValues) {
  ...
  addressBlock(cursor, formatFormKAddressLines(values.inspectorNameAddress), PAGE.marginX + 10, 4, true);
  cursor.y += 3;

  boldText(cursor, 'To:');
  addressBlock(cursor, formatFormKAddressLines(getKAddress(formType)), PAGE.marginX + 10, 5, true);
  cursor.y += 5;
  ...
  addressBlock(cursor, formatFormKAddressLines(values.inspectorNameAddress), PAGE.marginX + 10, 4);
...
function drawFormP(cursor: PdfCursor, values: FertilizerPdfValues) {
  ...
  cursor.y += 2;
  field(cursor, '6. Name & Address of Fertilizer\nInspector Drawing Sample', formatFormKAddressLines(values.inspectorNameAddress), 82);
