...
/**
 * Saves a jsPDF document as a named file. Uses a Blob + programmatic
 * `<a download>` click so it works on desktop, Android Chrome, iOS Safari
 * and installed-PWA mode (where `doc.save()` can silently fail).
 */
export async function savePdfDocument(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  doc: { output: (type?: any) => any },
  fileName: string,
): Promise<void> {
...
/**
 * Saves an XLSX workbook as a named file. Equivalent to `XLSX.writeFile`
 * but routed through the shared blob downloader so it also works on
 * mobile browsers and installed-PWA mode.
 */
export async function saveWorkbookFile(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  XLSX: { write: (workbook: any, options: any) => any },
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  workbook: any,
  fileName: string,
): Promise<void> {
  const bookType = fileName.toLowerCase().endsWith('.xls') ? 'xls' : 'xlsx';
  const data = XLSX.write(workbook, { bookType, type: 'array' }) as ArrayBuffer | Uint8Array;
  const mime = bookType === 'xls' ? 'application/vnd.ms-excel' : 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
  downloadBlobFile(new Blob([data], { type: mime }), fileName);
}
