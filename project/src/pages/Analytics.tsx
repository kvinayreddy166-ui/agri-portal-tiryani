  ...
  return (
    <div className="space-y-4">
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="border-t border-slate-100 px-3 py-2 text-xs font-bold text-slate-500">MAO/Admin Footer: Tiryani Agriculture Portal | Generated by {user?.email || 'MAO/Admin'}</div>
      </section>
    </div>
  );
}
...
function reportMetadataRows(title: string, fromDate: string, toDate: string, generatedBy: string) {
  return [
    ...
    ['Footer', 'MAO/Admin, Tiryani Agriculture Portal'],
  ];
}
...
function addFooter(doc: {
  ...
  doc.setFontSize(8);
  doc.text(`MAO/Admin, Tiryani Agriculture Portal | Generated by ${generatedBy}`, doc.internal.pageSize.getWidth() / 2, y, { align: 'center' });
}
