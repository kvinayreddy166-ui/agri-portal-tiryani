...
export function CropIntelligencePage({ cropSlug = 'paddy' }) {
  ...
  if (error || !crop) {
    return <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 font-semibold text-amber-900">Crop intelligence data is not available yet. Import the generated SQL or JSON first.</div>;
  }

  return (
    <div className="space-y-4">
      <section className="relative overflow-hidden rounded-xl">
        <img src={crop.image_url} alt={crop.name_en || crop.crop_name} className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative p-5 text-white md:p-7">
          ...
          <h1 className="mt-3 text-3xl font-black">{crop.name_en || crop.crop_name}</h1>
          <p className="text-sm italic text-emerald-100">{crop.scientific_name}</p>
          <p className="mt-3 max-w-4xl text-sm font-semibold leading-6 text-emerald-50">{profile.crop_profile}</p>
        </div>
      </section>

      <div className="grid gap-3 lg:grid-cols-3">
        <InfoTile icon={Sprout} label="Soil" value={profile.soil_requirements} />
        <InfoTile icon={Droplets} label="Irrigation" value={(crop.crop_irrigation || []).map((item) => `${item.stage}: ${item.recommendation_en}`).join(' ')} />
        <InfoTile icon={AlertTriangle} label="Seed" value={profile.seed_rate_seed_treatment} />
      </div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search crop chatbot knowledge..."
          ...
          <button onClick={runSearch} className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-black text-white">Search</button>
        </div>
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          {(results.length ? results : faqs.slice(0, 6)).map((faq) => (
            <article key={faq.id || faq.question} className="rounded-lg bg-slate-50 p-3 dark:bg-slate-800">
              ...
              <p className="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">{faq.answer}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section>
        <h2 className="mb-2 text-xl font-black text-slate-950 dark:text-white">Recommended Varieties</h2>
        <CropCards varieties={cards.varieties} />
      </section>

      <section>
        <h2 className="mb-2 text-xl font-black text-slate-950 dark:text-white">Fertilizer Schedule</h2>
        <FertilizerTable fertilizers={cards.fertilizers} />
      </section>

      <section>
        <h2 className="mb-2 text-xl font-black text-slate-950 dark:text-white">Pest Management</h2>
        <PestCards pests={cards.pests} />
      </section>

      <section>
        <h2 className="mb-2 text-xl font-black text-slate-950 dark:text-white">Disease Management</h2>
