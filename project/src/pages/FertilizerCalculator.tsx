...
export function FertilizerCalculator() {
  ...
  useEffect(() => {
    const loadCalculatorData = async () => {
      ...
      if (!gradeResponse.error && gradeResponse.data?.length) {
        ...
        setDataNotice('Supabase fertilizer tables are unavailable. Showing saved local defaults for now.');
      }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-3 text-slate-950">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-xl font-black sm:text-2xl">{t('Smart Fertilizer Calculator', 'స్మార్ట్ ఎరువుల కాలిక్యులేటర్')}</h1>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:flex">
            <button type="button" onClick={toggleLanguage} className="inline-flex items-center justify-center gap-2 rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm font-black text-emerald-800">
              {language === 'te' ? 'English' : 'తెలుగు'}
            </button>
            <button type="button" onClick={exportPdf} className="inline-flex items-center justify-center gap-2 rounded-lg bg-red-600 px-3 py-2 text-sm font-black text-white">
              <FileText className="h-4 w-4" /> PDF
            </button>
            <button type="button" onClick={exportExcel} className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-700 px-3 py-2 text-sm font-black text-white">
              <FileSpreadsheet className="h-4 w-4" /> Excel
            </button>
            <button type="button" onClick={shareWhatsApp} className="inline-flex items-center justify-center gap-2 rounded-lg bg-green-600 px-3 py-2 text-sm font-black text-white">
              <Send className="h-4 w-4" /> WhatsApp
            </button>
          </div>
        </div>
        {dataNotice && <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-bold text-amber-900">{dataNotice}</p>}
      </section>
      ...
      <section className="grid gap-3 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-xl border border-emerald-100 bg-white p-3 shadow-sm sm:p-4">
          <div className="mb-3 grid grid-cols-2 rounded-xl bg-slate-100 p-1 text-sm font-black">
            <button type="button" onClick={() => setMode('simple')} className={`rounded-lg px-3 py-2 ${mode === 'simple' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-600'}`}>
              Mode 1: Simple
            </button>
            <button type="button" onClick={() => setMode('crop')} className={`rounded-lg px-3 py-2 ${mode === 'crop' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-600'}`}>
              Mode 2: Crop
            </button>
          </div>
          ...
          {mode === 'simple' ? (
            <div className="space-y-3">
              <div className="grid grid-cols-2 rounded-xl bg-emerald-50 p-1 text-sm font-black">
                <button type="button" onClick={() => setSimpleTab('forward')} className={`rounded-lg px-3 py-2 ${simpleTab === 'forward' ? 'bg-white text-emerald-800 shadow-sm' : 'text-emerald-700'}`}>
                  Required to Fertilizer
                </button>
                <button type="button" onClick={() => setSimpleTab('reverse')} className={`rounded-lg px-3 py-2 ${simpleTab === 'reverse' ? 'bg-white text-emerald-800 shadow-sm' : 'text-emerald-700'}`}>
                  Bags to Nutrients
                </button>
              </div>
                ...
                <>
                  <div className="grid gap-2 sm:grid-cols-3">
                    {(['n', 'p', 'k'] as const).map((key) => (
                      <label key={key} className="block rounded-lg border border-slate-200 bg-slate-50 p-3">
                        ...
                        <input
                          ...
                          className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-lg font-black outline-none focus:border-emerald-500"
                        />
                      </label>
                    ))}
                  </div>
            ...
            <div className="space-y-3">
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <label className="block rounded-lg border border-slate-200 bg-slate-50 p-3 sm:col-span-2 lg:col-span-1">
        ...
        <div className="rounded-xl border border-emerald-100 bg-white p-3 shadow-sm sm:p-4">
          ...
          <div className="grid max-h-[23rem] gap-2 overflow-y-auto pr-1 sm:grid-cols-2">
            {grades.map((grade) => (
              <button
                key={gradeKey(grade)}
                type="button"
      ...
      <section className="grid gap-3 lg:grid-cols-[0.8fr_1.2fr]">
        <div className="rounded-xl border border-emerald-100 bg-white p-3 shadow-sm sm:p-4">
          <h2 className="mb-3 text-base font-black">{simpleTab === 'reverse' && mode === 'simple' ? 'Reverse Calculation' : 'Live Calculation'}</h2>
        ...
        <div className="rounded-xl border border-emerald-100 bg-white p-3 shadow-sm sm:p-4">
          <h2 className="mb-3 text-base font-black">Fertilizer Quantity and Breakdown</h2>
      ...
      {mode === 'crop' && (
        <section className="rounded-xl border border-emerald-100 bg-white p-3 shadow-sm sm:p-4">
          <h2 className="mb-3 text-base font-black">Smart Split Dose Planner - Fertilizer Quantities</h2>
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
            {splitFertilizerPlan.map(({ dose, nutrients, calculation: doseCalculation }) => (
              <div key={dose.stage} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
      ...
      <section className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-amber-950 shadow-sm sm:p-4">
          <h2 className="mb-2 flex items-center gap-2 text-base font-black"><Leaf className="h-5 w-5" /> Important Agronomic Note</h2>
          <ul className="space-y-1 text-sm font-bold">
            <li>Apply 3-4 tons FYM per acre as basal application.</li>
            <li>Fertilizer recommendations should preferably be based on Soil Test Results.</li>
            <li>Split application of Nitrogen and Potash improves fertilizer use efficiency.</li>
