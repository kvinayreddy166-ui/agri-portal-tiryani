...
function buildWhatsAppText(input: FertilizerReportInput) {
  ...
  const labels = language === 'te' ? repairTeluguRecord({
    title: 'స్మార్ట్ ఎరువుల కాలిక్యులేటర్',
    subtitle: 'అగ్రోనిక్స్',
    ...
    variety: 'రకం',
    area: 'ప్రాంతం',
    customNutrientDose: 'కస్టమ్ న్యూట్రియంట్ డోస్',
    recommendedNutrients: 'ఎకరాకు సిఫార్సు చేసిన పోషకాలు',
    totalRequirement: 'మొత్తం అవసరం',
    fertilizerReq: 'ఎరువుల అవసరం',
    noFertilizer: 'ఎరువులు అవసరం లేదు',
    nutrientBalance: 'పోషకాల సమతుల్యత',
    supplied: 'అందించినది',
    balance: 'సమతుల్యత',
    excess: 'అదనపు',
    splitDosePlan: 'స్ప్లిట్ డోస్ ప్లాన్',
    target: 'లక్ష్యం',
    note: 'గమనిక',
    gypsum: 'జిప్సం',
    topDressing: 'టాప్ డ్రెస్సింగ్',
    noFertilizerSplit: 'ఈ స్ప్లిట్‌లో ఎరువులు అవసరం లేదు',
    importantNotes: 'ముఖ్యమైన వ్యవసాయ గమనికలు',
    generatedBy: 'తయారు చేసినవారు',
    officer: 'కె. వినయ్ రెడ్డి, ఎంఏఓ, తిర్యాని',
    mandal: 'తిర్యాని మండల్, కుమ్రం భీమ్ ఆసిఫాబాద్ జిల్లా',
    tryIt: 'దీనిని ప్రయత్నించండి:',
  }) : {
    ...
    variety: 'Variety',
    area: 'Area',
    customNutrientDose: 'Custom nutrient dose',
    recommendedNutrients: 'Recommended Nutrients Per Acre',
    totalRequirement: 'Total Nutrient Requirement',
    fertilizerReq: 'Fertilizer Requirements',
    noFertilizer: 'No fertilizer required',
    nutrientBalance: 'Nutrient Balance',
    supplied: 'Supplied',
    balance: 'Balance',
    excess: 'Excess',
    splitDosePlan: 'Split Dose Plan',
    target: 'Target',
    note: 'Note',
    gypsum: 'Gypsum',
    topDressing: 'Top dressing',
    noFertilizerSplit: 'No fertilizer required in this split',
    importantNotes: 'Important Notes',
    generatedBy: 'Generated by',
    officer: 'K. Vinay Reddy, MAO, Tiryani',
    mandal: 'Tiryani Mandal, Kumram Bheem Asifabad District',
    tryIt: 'Try it at:',
  };
  ...
  const farmerLines = [
    farmerDetails?.farmerName ? `${labels.farmer}: ${farmerDetails.farmerName}` : '',
    farmerDetails?.village ? `${labels.village}: ${farmerDetails.village}` : '',
    farmerDetails?.ppbNumber ? `${labels.ppb}: ${farmerDetails.ppbNumber}` : '',
    farmerDetails?.surveyNumber ? `${labels.surveyNo}: ${farmerDetails.surveyNumber}` : '',
    farmerDetails?.mobileNumber ? `${labels.mobile}: ${farmerDetails.mobileNumber}` : '',
    farmerDetails?.acreage ? `${labels.farmerArea}: ${farmerDetails.acreage} ${labels.acre}` : '',
  ].filter(Boolean);

  if (farmerLines.length) {
    lines.push(`*${labels.farmerDetails}*`, ...farmerLines, separator);
  }

  lines.push(
    `*${labels.cropDetails}*`,
    mode === 'crop' ? `${labels.crop}: ${crop}` : `${labels.crop}: ${labels.customNutrientDose}`,
    mode === 'crop' ? `${labels.season}: ${selectedRecommendation.season}` : '',
    mode === 'crop' ? `${labels.zone}: ${selectedRecommendation.zone}` : '',
    mode === 'crop' ? `${labels.variety}: ${selectedRecommendation.variety}` : '',
    `${labels.area}: ${formatSelectedArea(area, language)}`,
    separator,
    `*${labels.recommendedNutrients}*`,
  ...
  if (fertilizerRows.length) {
    ...
    lines.push(labels.noFertilizer);
  }

  lines.push(
    separator,
    `*${labels.nutrientBalance}*`,
    `${labels.supplied} - N: ${round(supplied.n)} kg, ${formatNutrientLabel('p')}: ${round(supplied.p)} kg, ${formatNutrientLabel('k')}: ${round(supplied.k)} kg`,
    `${labels.balance} - N: ${round(balance.n)} kg, ${formatNutrientLabel('p')}: ${round(balance.p)} kg, ${formatNutrientLabel('k')}: ${round(balance.k)} kg`,
    `${labels.excess} - N: ${round(excess.n)} kg, ${formatNutrientLabel('p')}: ${round(excess.p)} kg, ${formatNutrientLabel('k')}: ${round(excess.k)} kg`
  );

  if (mode === 'crop' && splitPlan.length) {
    lines.push(separator, `*${labels.splitDosePlan}*`);
    splitPlan.forEach(({ dose, nutrients, calculation }, index) => {
      ...
      lines.push(`${labels.target} N: ${round(nutrients.n)} kg, ${formatNutrientLabel('p')}: ${round(nutrients.p)} kg, ${formatNutrientLabel('k')}: ${round(nutrients.k)} kg`);
      if (dose.notes) lines.push(`${labels.note}: ${dose.notes}`);
      if (dose.gypsum_kg) lines.push(`${labels.gypsum}: ${dose.gypsum_kg} kg`);
      if (dose.top_dressing_n_kg) lines.push(`${labels.topDressing}: ${dose.top_dressing_n_kg} kg N`);
      if (rows.length) {
        rows.forEach((row) => lines.push(`${row.grade.name}: ${round(row.kg)} kg (${round(row.bags)} bags)`));
      } else {
        lines.push(labels.noFertilizerSplit);
      }
    });
  }
...
export function FertilizerCalculator() {
  ...
  const exportPdf = async () => {
    ...
    const generatePdfWithAutoFit = async (autoFitLevel: AutoFitLevel): Promise<boolean> => {
      ...
      doc.text('AGRONIX | Generated by K. Vinay Reddy, MAO, Tiryani', pageWidth / 2, pageHeight - 16, { align: 'center' });
      doc.setFontSize(7);
  ...
  const shareWhatsApp = () => {
    try {
      ...
      if (!activeCalculation || !activeCalculation.results || activeCalculation.results.length === 0) {
        alert('Please perform a calculation before sharing on WhatsApp.');
        return;
      }
      ...
      const text = buildWhatsAppText({
        ...
        balance: activeCalculation.balance,
        excess: activeCalculation.excess,
        selectedRecommendation: selectedRecommendation || { crop: 'Custom', n: 0, p: 0, k: 0, season: '', zone: '', variety: '' },
        recommendationNutrients: recommendationNutrients || activeRequired,
        splitPlan: splitFertilizerPlan || [],
        farmerDetails,
      });
      ...
      alert(`Failed to share on WhatsApp: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
