...
export function FarmerDatabase() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      let query = supabase
        ...
        .select(farmerSelectColumns(true, true), { count: 'exact' })
      ...
      if (error && isMissingColumnError(error)) {
        query = supabase
          .from('farmer_database')
          .select(farmerSelectColumns(true, false), { count: 'exact' })
          .order('village_english')
          .order('farmer_name_english')
          .range(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE - 1);
        query = applyFarmerSearch(query, debouncedSearch);
  ...
  const exportFiltered = async () => {
    ...
    const uniqueFarmers = new Set(analyticsRows.map((row) => row.identity_key || farmerIdentityKey(row))).size;
    const totalExtent = guntasToExtent(analyticsRows.reduce((sum, row) => sum + extentToGuntas(Number(row.extent || 0)), 0));
    const summaryRows = [
      ['Tiryani Agriculture Portal'],
      ['Farmer Database Export'],
      [],
      ['Generated By', 'K. Vinay Reddy, MAO, Tiryani'],
      ['Generated On', new Date().toLocaleString('en-IN')],
      ['Website / App', 'Tiryani Agriculture Portal'],
      ...
      ['Total Cultivable Area (Acres)', formatExtent(totalExtent)],
      ['Total Villages', new Set(analyticsRows.map((row) => row.village_english).filter(Boolean)).size],
      ['Total Crops', new Set(analyticsRows.map((row) => row.crop).filter(Boolean)).size],
      [],
      ['Note', 'Data exported from existing app database/farmer workbook records.'],
    ];
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <label className="action-button cursor-pointer">
                ...
                {uiLabel('Upload XLSX/CSV', showTelugu)}
              ...
              <button type="button" onClick={replaceDatabase} disabled={!previewRows.length || importing} className="action-button bg-emerald-700 text-white disabled:opacity-50">
                <Save className="h-4 w-4" /> {uiLabel('Replace', showTelugu)}
              </button>
            </div>
          </div>
...
function groupFarmerRows(rows: FarmerRow[]): FarmerGroup[] {
  ...
  rows.forEach((row) => {
    ...
    current.totalExtent += extentToGuntas(Number(row.extent || 0));
    const crop = current.cropRows.find((item) => item.crop === row.crop);
    if (crop) crop.extent += extentToGuntas(Number(row.extent || 0));
    else current.cropRows.push({ crop: row.crop || 'Not specified', extent: extentToGuntas(Number(row.extent || 0)) });
    map.set(key, current);
  });
...
function buildAnalytics(rows: FarmerRow[]) {
  ...
  rows.forEach((row) => {
    ...
    villageExtent.set(village, (villageExtent.get(village) || 0) + extentToGuntas(Number(row.extent || 0)));
    cropExtent.set(crop, (cropExtent.get(crop) || 0) + extentToGuntas(Number(row.extent || 0)));
  });
  return {
...
function buildFarmerSummary(rows: FarmerRow[]) {
  ...
  const totalExtent = rows.reduce((sum, row) => {
    ...
    return sum + extentToGuntas(Number(row.extent || 0));
  }, 0);
  return {
...
function cropDisplay(crop: string, showTelugu: boolean) {
  ...
  const labels: Record<string, string> = {
    ...
    Greengram: 'పెసర',
    'Greengram(Green Manure)': 'పచ్చి ఎరువు పెసర',
    Guava: 'జామ',
    Jowar: 'జొన్న',
    Maize: 'మొక్కజొన్న',
    Mango: 'మామిడి',
...
function uiLabel(label: string, showTelugu: boolean) {
  ...
  const labels: Record<string, string> = {
    ...
    'Search by farmer, family name, phone, PPB, Aadhaar, survey number, or village.': 'రైతు పేరు, కుటుంబ పేరు, ఫోన్, పి.పి.బి, ఆధార్, సర్వే నంబర్ లేదా గ్రామం ద్వారా వెతకండి.',
    'Search by farmer name, phone, PPB, Aadhaar, survey no or village': 'రైతు పేరు, ఫోన్, పి.పి.బి, ఆధార్, సర్వే నంబర్ లేదా గ్రామం ద్వారా వెతకండి',
    ...
    'Cultivable area from farmer database': 'రైతుల డేటాబేస్ ప్రకారం సాగు విస్తీర్ణం',
    'Admin Import Tools': 'అడ్మిన్ దిగుమతి సాధనాలు',
    'Upload CSV/XLSX, preview, then replace the farmer database.': 'CSV/XLSX ఫైల్ అప్‌లోడ్ చేసి, ముందుగా చూసి, తరువాత రైతుల డేటాబేస్‌ను మార్చండి.',
    Template: 'నమూనా',
    'Upload XLSX/CSV': 'XLSX/CSV అప్‌లోడ్',
    Replace: 'మార్చండి',
    Records: 'రికార్డులు',
    Villages: 'గ్రామాలు',
    Crops: 'పంటలు',
    'Unique Farmers': 'ప్రత్యేక రైతులు',
    Previous: 'మునుపటి',
    Next: 'తదుపరి',
    Page: 'పేజీ',
    'Call farmer': 'రైతుకు కాల్ చేయండి',
    'WhatsApp farmer': 'రైతుకు వాట్సాప్ చేయండి',
    'View details': 'వివరాలు చూడండి',
    'Update phone number': 'ఫోన్ నంబర్ మార్చండి',
    'Add remarks': 'గమనికలు జోడించండి',
    Save: 'సేవ్ చేయండి',
    [emptyStateText]: 'రైతు రికార్డులు కనబడలేదు. పేరు, ఫోన్ నంబర్, పి.పి.బి లేదా గ్రామంతో ప్రయత్నించండి.',
    Survey: 'సర్వే',
    Phone: 'ఫోన్',
    Aadhaar: 'ఆధార్',
    PPB: 'పి.పి.బి',
