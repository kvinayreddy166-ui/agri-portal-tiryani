    ...
    <div className={`flex flex-col ${className}`}>
      <div className="mb-2 flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-2">
        <div className="flex items-center gap-2">
          ...
          {sheets.length === 1 && (
            <span className="text-sm font-semibold text-slate-900 dark:text-white">{currentSheet?.name || 'Sheet'}</span>
          )}
        </div>
        ...
        {loading && (
          <div className="flex h-full items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">Loading spreadsheet...</p>
            </div>
          </div>
        )}
              ...
              <thead>
                <tr className="bg-slate-100 dark:bg-slate-800">
                  {currentSheet.headers.map((header, index) => (
                    <th
                      key={index}
                      className="border border-slate-300 dark:border-slate-600 px-3 py-2 text-left font-semibold text-slate-900 dark:text-white"
                    >
                      {header}
                    </th>
                  ))}
                ...
                {currentSheet.data.map((row, rowIndex) => (
                  <tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                    {row.map((cell, cellIndex) => (
                      <td
                        key={cellIndex}
                        className="border border-slate-300 dark:border-slate-600 px-3 py-2 text-slate-700 dark:text-slate-200"
                      >
                        {cell}
                      </td>
                    ))}
