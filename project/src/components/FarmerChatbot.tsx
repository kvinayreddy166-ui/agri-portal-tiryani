...
import { loadSpeechVoices, pickFemaleIndianVoice } from '../lib/speechVoice';
...
async function speakText(text: string, lang: AdvisoryLang) {
  ...
  const voice = pickFemaleIndianVoice(voices, lang);
...
export function FarmerChatbot({ showOnLoginPage = false }: FarmerChatbotProps) {
  ...
  const sendMessage = (text: string) => {
    ...
    setInput('');
    if (voiceReply) void speakText(reply.text, reply.lang);
  };
      ...
      >
        <div className="flex items-center gap-1">
          <button
            ...
            title={t('Voice input (Telugu)', 'వాయిస్ ఇన్‌పుట్ (తెలుగు)')}
          >
